...
import cn.ucai.secret.activity.AddContactActivity;
import cn.ucai.secret.activity.ChatActivity;
import cn.ucai.secret.activity.GroupsActivity;
import cn.ucai.secret.activity.MainActivity;
import cn.ucai.secret.activity.NewFriendsMsgActivity;
import cn.ucai.secret.activity.PublicChatRoomsActivity;
import cn.ucai.secret.activity.RobotsActivity;
import cn.ucai.secret.applib.controller.HXSDKHelper;
import cn.ucai.secret.applib.controller.HXSDKHelper.HXSyncListener;
import com.easemob.chat.EMContactManager;
import cn.ucai.secret.Constant;
import cn.ucai.secret.DemoHXSDKHelper;
import cn.ucai.secret.adapter.ContactAdapter;
import cn.ucai.secret.db.InviteMessgeDao;
import cn.ucai.secret.db.UserDao;
import cn.ucai.secret.domain.User;
import cn.ucai.secret.widget.Sidebar;
...
public class ContactlistFragment extends Fragment {
				    ...
				    getActivity().runOnUiThread(new Runnable(){
		                ...
		                public void run() {
		                    if(success){
		                        ...
		                        String s1 = getResources().getString(cn.ucai.secret.R.string.get_failed_please_check);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		return inflater.inflate(cn.ucai.secret.R.layout.fragment_contact_list, container, false);
	}
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		listView = (ListView) getView().findViewById(cn.ucai.secret.R.id.list);
		sidebar = (Sidebar) getView().findViewById(cn.ucai.secret.R.id.sidebar);
		...
		//搜索框
		query = (EditText) getView().findViewById(cn.ucai.secret.R.id.query);
		query.setHint(cn.ucai.secret.R.string.search);
		clearSearch = (ImageButton) getView().findViewById(cn.ucai.secret.R.id.search_clear);
		...
		// 设置adapter
		adapter = new ContactAdapter(getActivity(), cn.ucai.secret.R.layout.row_contact, contactList);
		...
		ImageView addContactView = (ImageView) getView().findViewById(cn.ucai.secret.R.id.iv_new_contact);
		// 进入添加好友页
		...
		progressBar = (View) getView().findViewById(cn.ucai.secret.R.id.progress_bar);
	...
	public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
		...
		if (((AdapterContextMenuInfo) menuInfo).position > 3) {
			...
			getActivity().getMenuInflater().inflate(cn.ucai.secret.R.menu.context_contact_list, menu);
		}
	}

	@Override
	public boolean onContextItemSelected(MenuItem item) {
		if (item.getItemId() == cn.ucai.secret.R.id.delete_contact) {
			try {
                // 删除此联系人
			...
			return true;
		}else if(item.getItemId() == cn.ucai.secret.R.id.add_to_blacklist){
	...
	public void deleteContact(final User tobeDeleteUser) {
		String st1 = getResources().getString(cn.ucai.secret.R.string.deleting);
		final String st2 = getResources().getString(cn.ucai.secret.R.string.Delete_failed);
	...
	private void moveToBlacklist(final String username){
		final ProgressDialog pd = new ProgressDialog(getActivity());
		String st1 = getResources().getString(cn.ucai.secret.R.string.Is_moved_into_blacklist);
		final String st2 = getResources().getString(cn.ucai.secret.R.string.Move_into_blacklist_success);
		final String st3 = getResources().getString(cn.ucai.secret.R.string.Move_into_blacklist_failure);
		pd.setMessage(st1);
		pd.setCanceledOnTouchOutside(false);
		pd.show();
		new Thread(new Runnable() {
